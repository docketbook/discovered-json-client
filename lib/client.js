...
function getAddresesForService(service, callback) {
	...
	if (statusObj.status === 'waiting') {
		...
		performGetAddressForService(service, function(err, endpoints) {
			refreshStatus.callbacks.forEach(function(callback) {
				return callback(err, endpoints);
			});
			refreshStatus.callbacks.splice(0, refreshStatus.callbacks.length);
...
exports.init = function(consul, restify, options) {
	...
	if ('refreshInterval' in options) {
		refreshWindow = options.refreshWindow;
	}
	if ('maximumConcurrentUpdates' in options) {
		maximumConcurrentUpdates = options.maximumConcurrentUpdates;
	}
	setRefreshTimer();
}